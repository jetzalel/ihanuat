...
public class AutoSellManager {
    ...
    public static void onAutoSellDetected(Minecraft client) {
        client.player.displayClientMessage(Component.literal("§eAutoSell: Detected trigger message"), true);
        
        if (!MacroConfig.autoSell) {
            client.player.displayClientMessage(Component.literal("§cAutoSell: Feature disabled"), true);
            return;
        }
        
        if (!com.ihanuat.mod.MacroStateManager.isMacroRunning()) {
            client.player.displayClientMessage(Component.literal("§cAutoSell: Macro not running"), true);
            return;
        }
        ...
        if (GearManager.isSwappingWardrobe || GearManager.isSwappingEquipment ||
            ...
            client.player.displayClientMessage(Component.literal("§cAutoSell: Gear/pest swap in progress"), true);
            return;
        }

        if (VisitorManager.getVisitorCount(client) >= MacroConfig.visitorThreshold) {
            client.player.displayClientMessage(Component.literal("§cAutoSell: Visitor threshold met"), true);
            return;
        }

        // Don't auto-sell if already in the middle of an auto sell sequence
        if (isPreparingToAutoSell || isAutoSelling) {
            client.player.displayClientMessage(Component.literal("§cAutoSell: Already running"), true);
            return;
        }
        ...
        if (MacroStateManager.getCurrentState() != MacroState.State.FARMING &&
            ...
            client.player.displayClientMessage(Component.literal("§cAutoSell: Wrong state: " + MacroStateManager.getCurrentState()), true);
            return;
        }

        client.player.displayClientMessage(Component.literal("§aAutoSell: Starting sequence"), true);
    ...
    private static void triggerAutoSell(Minecraft client) {
        ...
        new Thread(() -> {
            try {
                client.player.displayClientMessage(Component.literal("§eAutoSell: Stopping script..."), true);
                ClientUtils.sendCommand(client, ".ez-stopscript");

                boolean success = true;
                client.player.displayClientMessage(Component.literal("§eAutoSell: Waiting for script stop (5 seconds)..."), true);
                for (int i = 0; i < 50; i++) {
                    Thread.sleep(100);
                    if (!isPreparingToAutoSell) {
                        success = false;
                        client.player.displayClientMessage(Component.literal("§cAutoSell: Prep cancelled"), true);
                        break;
                    }
                    if (MacroStateManager.getCurrentState() != MacroState.State.FARMING) {
                        success = false;
                        client.player.displayClientMessage(Component.literal("§cAutoSell: Not in FARMING state"), true);
                        break;
                    }
                }
                ...
                if (success && isPreparingToAutoSell) {
                    ...
                    client.player.displayClientMessage(Component.literal("§eAutoSell: Opening trades menu..."), true);
                    ClientUtils.sendCommand(client, "/trades");
                } else {
                    isPreparingToAutoSell = false;
                    client.player.displayClientMessage(Component.literal("§cAutoSell: Failed to start"), true);
                }
            } catch (Exception e) {
                e.printStackTrace();
                client.player.displayClientMessage(Component.literal("§cAutoSell: Exception occurred"), true);
            }
        }).start();
    }
